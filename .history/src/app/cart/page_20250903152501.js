...
const CartPage = () => {
  const [cartData, setCartData] = useState(null); // store full response
          ...
          {cartItems.map((item, idx) => (
            ...
            >
              ...
              <div className="flex-1 space-y-2">
                <h2 className="text-xl font-semibold">{item.name || 'Unnamed Shoe'}</h2>
                <p className="text-sm text-gray-400">Size: {item.size}</p>
                <p className="text-sm text-gray-400">Color: {item.variantColor}</p>
