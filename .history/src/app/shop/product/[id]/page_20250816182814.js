  ...
  return (
    <div className="bg-black text-white min-h-screen px-6 py-10">
      <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-10">
        ...
        {/* Recommended Section */} {recommended?.length > 0 && ( <div className="mt-12"> <h2 className="text-2xl font-semibold mb-4">Recommended for You</h2> <div className="grid grid-cols-2 md:grid-cols-4 gap-4"> {recommended.map((shoe) => ( <div key={shoe.id} className="bg-[#121212] p-3 rounded-xl"> <img src={shoe.image} alt={shoe.name} width={250} height={160} className="rounded object-cover w-full h-[160px]" /> <h3 className="text-lg font-bold mt-2">{shoe.name}</h3> <p className="text-gray-400">{shoe.brand}</p> <p className="text-red-500 font-semibold mt-1">₹{shoe.price}</p> </div> ))} </div> </div> )} </div> );
      </div>
    </div>
  );
}
